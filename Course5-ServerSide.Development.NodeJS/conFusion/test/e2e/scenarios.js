...
describe("menu 0 item", function() {
  ...
  it("should show the number of comments as", function() {
    expect(
      element.all(by.repeater("comments in dish.comments")).count()).toEqual(5);
  });

  it("should show the first comment author as", function() {
    element(by.model("sortBy")).sendKeys("author");
    expect(
      element.all(by.repeater("comments in dish.comments")).count()
    ).toEqual(5);
    var author = element.all(
      by.repeater("comments in dish.comments")).first().element(
        by.binding("comments.author")
      );
